# -*- coding: utf-8 -*-
"""
Created on Sat Aug 29 18:21:12 2020
...
def data_read(symbol,start,end):
    ...
    data[symbol] = get_history(symbol=symbol, start=date(2012,1,1),end=date(2020,8,28))['Close']
    data[bse] = web.DataReader(bse, data_source='yahoo', start='2012-1-1')['Adj Close']      
...
def holt_time(df):
    ...
    train_data = df.iloc[:lt-3]# Goes up to but not including 108
    test_data = df.iloc[lt-3:]
...
def file_write(symbol):
    df = pd.DataFrame()
    df = get_history(symbol=symbol, start=date(2012,1,1),end=date(2020,8,28))
    df.to_csv('test.csv')
...
symbol = st.sidebar.selectbox('Comapny Symbol for NSE',lsta, key='1')
st.sidebar.title("User Inputs")
start = st.sidebar.text_input("Start Date","2012-1-1")
end = st.sidebar.text_input("End Date","2020-8-29")


ticker =symbol

Company = all_stock_codes[symbol]

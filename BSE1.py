# -*- coding: utf-8 -*-
"""
Created on Sun Sep  6 13:37:49 2020
...
def holt_time(ticker):
    
    start = datetime.datetime(2015, 1, 1)
    df1 = web.DataReader(ticker, 'yahoo', start)
    ...
    train_data = df1.iloc[:lt-2]# Goes up to but not including 108
    test_data = df1.iloc[lt-2:]
    ...
    final_model = ExponentialSmoothing(df1['Adj Close'],trend='mul',seasonal='mul',seasonal_periods=12).fit()
...
def file_write(symbol):
    ...
    df = web.DataReader(symbol, data_source='yahoo', start='2015-1-1')
    df.to_csv('test1.csv')
...
def day_chart(symbol):
    
    data = yf.download(tickers=symbol, period="60d", interval="15m")

    data['5DMA'] = data['Close'].rolling(window=5).mean()
    data['20DMA'] = data['Close'].rolling(window=20).mean()
    data = MACD(data, 12, 26, 9)
    data = BollBnd(data,20)
    data['VWAP'] = vwap(data)
    X = data
...
#Set up traces
    fig.add_trace(go.Scatter(x=X['Datetime'], y= X['MA'],line=dict(color='blue', width=.7), name = 'Middle Band'))
    fig.add_trace(go.Scatter(x=X['Datetime'], y= X['BB_up'],line=dict(color='red', width=1.5), name = 'Upper Band (Sell)'))
    fig.add_trace(go.Scatter(x=X['Datetime'], y= X['BB_dn'],line=dict(color='green', width=1.5), name = 'Lower Band (Buy)'))
    fig.add_trace(go.Scatter(x=X['Datetime'], y= X['Signal'],line=dict(color='Orange', width=1.5), name = 'Singal'))
    fig.add_trace(go.Scatter(x=X['Datetime'], y= X['MACD'],line=dict(color='black', width=1.5), name = 'Slow'))
    fig.add_trace(go.Scatter(x=X['Datetime'], y= X['VWAP'],line=dict(color='yellow', width=1.5), name = 'VWAP'))
    fig.add_trace(go.Scatter(x=X['Datetime'], y= X['5DMA'],line=dict(color='violet', width=1.5), name = '5DMA'))
    fig.add_trace(go.Scatter(x=X['Datetime'], y= X['20DMA'],line=dict(color='grey', width=1.5), name = '20DMA'))

              

    fig.add_trace(go.Candlestick(x=X['Datetime'],
                    open=X['Open'],
                    high=X['High'],
                    low=X['Low'],
                    close=X['Close'], name = 'market data'))
    # Add titlesn
    fig.update_layout(
...
#Set up traces
    fig.add_trace(go.Scatter(x=X['Date'], y= X['MA'],line=dict(color='blue', width=.7), name = 'Middle Band'))
    fig.add_trace(go.Scatter(x=X['Date'], y= X['BB_up'],line=dict(color='red', width=1.5), name = 'Upper Band (Sell)'))
    fig.add_trace(go.Scatter(x=X['Date'], y= X['BB_dn'],line=dict(color='green', width=1.5), name = 'Lower Band (Buy)'))
    fig.add_trace(go.Scatter(x=X['Date'], y= X['Signal'],line=dict(color='Orange', width=1.5), name = 'Singal'))
    fig.add_trace(go.Scatter(x=X['Date'], y= X['MACD'],line=dict(color='black', width=1.5), name = 'Slow'))
    fig.add_trace(go.Scatter(x=X['Date'], y= X['VWAP'],line=dict(color='yellow', width=1.5), name = 'VWAP'))
    fig.add_trace(go.Scatter(x=X['Date'], y= X['50DMA'],line=dict(color='violet', width=1.5), name = '50DMA'))
    fig.add_trace(go.Scatter(x=X['Date'], y= X['200DMA'],line=dict(color='grey', width=1.5), name = '200DMA'))
    ...
    fig.add_trace(go.Candlestick(x=X['Date'],
                    open=X['Open'],
                    high=X['High'],
                    low=X['Low'],
                    close=X['Close'], name = 'market data'))
...
symbol = st.sidebar.text_input("Ticker in capital","SUYOG.BO")
st.sidebar.title("User Inputs")
start = st.sidebar.text_input("Start Date","2015-1-1")
end = st.sidebar.text_input("End Date","2020-9-31")
...
if symbol != " ":
    ...
    data[symbol] = web.DataReader(symbol, data_source='yahoo', start='2015-1-1')['Adj Close']
    monta_carlo(data)
    ...
    #st.line_chart(df[["MACD","Signal","BB_up",'MA',"BB_dn","50DMA","200DMA",'VWAP']])
...
#Create the "%D" column in the DataFrame
    ...
    df['OBV'] = temp3
    df["RV"] = df["Volume"]/(df["Volume"].mean())
    st.line_chart(df[["Volume","RV","OBV"]])
